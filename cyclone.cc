...
Cyclone::Cyclone(const Cyclone &orig){
 ...
 void Cyclone::parseTableInput(string& input){
   ...
   int * varSpeeds = new int[num_vars];
   tables = new vector<Table>();
...
// file.
 void Cyclone::run(int cores){
   ...
   cores = 1;
   edgeArray = new unlong[total_states];
   ...
   if (!usingSpeeds){
     subprocessRun(maxState);
   }
   else {
     subprocessRunWithSpeeds(maxState);
   }
   cerr << "Running" << endl;
...
// edgeArray.
void Cyclone::subprocessRun(unlong maxState){
...
void Cyclone::subprocessRunWithSpeeds(unlong maxState){
...
void Cyclone::generateEdges(bool writeFile, string filename, bool verbose){
  ...
  outputEdges(writeFile,maxState, edges, filename, verbose);
  delete edges;
  delete ternState;
  delete ternNextState;
}
...
// filename
void Cyclone::outputEdges(bool writeFile, unlong maxState, unsigned int * edges, string filename, bool verbose){
...
void Cyclone::outputTrajectory(vector<unlong> * path, int cycleIndex, bool writeFile, string outFilename, bool verbose, string tableFilename){
  uchar * ternState = new uchar[num_vars];
  for (int i = 0; i < num_vars; i++){
    ternState[i] = 0;
  }
  ...
  if (writeFile){
    outFile << modelname << endl;
    outFile << runname << endl;
    outFile << "TRAJECTORY" << endl;
    outFile << "NONLOOPING: " << endl;
    outFile << "Length: " << cycleIndex << endl;
  }
  ...
  if (writeFile){
    outFile << "LIMIT CYCLE: " << endl;
    outFile << "Length: " << path->size() - cycleIndex << endl;
  }
  ...
  if (writeFile){
    outFile << "TABLE SOURCE: " << tableFilename << endl;
    outFile << "<NOTES>\n</NOTES>" << endl;
  }
  if (writeFile){
    outFile.close();
...
void Cyclone::outputRandomTrajectory(vector<unlong> * path[], bool writeFile, string outFilename, bool verbose, string tableFilename, int numSteps, int numRuns){
  uchar * ternState = new uchar[num_vars];
  for (int i = 0; i < num_vars; i++){
    ternState[i] = 0;
  }
  ...
  if (writeFile){
    outFile << modelname << endl;
    outFile << runname << endl;
    outFile << "RANDOM TRAJECTORY" << endl;
    outFile << "Runs: " << numRuns << endl;
    outFile << "Steps: " << numSteps << endl;
  }
...
void Cyclone::generateTrajectory(bool writeFile, string outFilename, bool verbose, string inFilename, string tableFilename)
{
  ...
  if (inFile.is_open()){
    ...
    outputTrajectory(path, cycleIndex, writeFile, outFilename, verbose, tableFilename);
...
void Cyclone::generateRandomTrajectory(bool writeFile, string outFilename, bool verbose, string inFilename, string tableFilename, int numRuns, int numSteps)
{
  ifstream inFile(inFilename.c_str());
  ...
  if (inFile.is_open()){
    ...
    outputRandomTrajectory(path, writeFile, outFilename, verbose, tableFilename, numSteps, numRuns);
